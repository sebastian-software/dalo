...
mod tests {
    ...
    #[test]
    fn command_failed_should_keep_the_public_fields_and_error_text() {
        let error = DaloError::CommandFailed {
            ...
            status: "128".to_owned(),
            stderr:
                "Could not clone repository `team\u{7}`.\n\nGit said: fatal: \u{1b}[2Jbad\noutput"
                    .to_owned(),
        };
    ...
    #[test]
    fn command_failed_should_preserve_empty_stderr() {
        let error = DaloError::CommandFailed {
            program: "git".to_owned(),
            args: "clone".to_owned(),
            cwd: PathBuf::from("/tmp/checkout"),
        ...
        assert_eq!(error.to_string(), "command `git` failed with status 128: ");
    }
