...
#[command(
    version,
    about = "One source of truth for the skills your AI agents run.",
    long_about = "One source of truth for the skills your AI agents run.\n\nGit-backed skill management for AI agents: Dalo keeps a local store of skill sources, resolves one approved skill set, and links that set into the folders your agents already read.",
...
fn run_tool(options: &GlobalOptions, command: ToolCommand) -> DaloResult<()> {
    ...
    match command.command {
        ...
        ToolSubcommand::Show(args) => {
            ...
            println!(
                "contract hash: {}",
                status::terminal_safe_text(&report.tool.contract_hash)
            );
            println!(
                "plugin package hash: {}",
                status::terminal_safe_text(&report.plugin_package_hash)
            );
            println!(
                "entry: {} ({})",
            ...
            println!(
                "capabilities: {}",
                status::terminal_safe_text(&format_display_values(&report.tool.capabilities))
            );
            println!(
                "diagnostic: {}",
...
fn run_hook(options: &GlobalOptions, command: HookCommand) -> DaloResult<()> {
    ...
    match command.command {
        ...
        HookSubcommand::Show(args) => {
            ...
            println!(
                "contract hash: {}",
                status::terminal_safe_text(&report.hook.contract_hash)
            );
            println!(
                "tool: {}",
            ...
            println!(
                "tool contract hash: {}",
                status::terminal_safe_text(&report.hook.tool_contract_hash)
            );
            println!(
                "event: {}.{} effect={}",
            ...
            println!(
                "bindings: {}",
                status::terminal_safe_text(&format_display_values(
                    &report.hook.descriptor.bindings
                ))
            );
            println!(
                "diagnostic: {}",
...
fn contextualize_team_cli_error(error: DaloError, repo: &std::path::Path) -> DaloError {
    let DaloError::InvalidStorePath { path, reason } = error else {
        return error;
    };
    if !reason.starts_with("team repository ") {
        return DaloError::InvalidStorePath { path, reason };
    }
    let origin = (repo != std::path::Path::new(".")).then_some(" (from --repo)");
    let message = format!(
        "team repository `{}` {}{}",
        path.display(),
        reason.trim_start_matches("team repository "),
        origin.unwrap_or_default()
    );
    DaloError::Io(std::io::Error::other(message))
}
