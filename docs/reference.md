...
### `dalo source add <id> <git-url-or-path> [--namespace <prefix>] [--ref <git-ref>] [--subpath <directory>]`
...
directory.
`--subpath` scans only a directory inside the checkout. It must be a non-empty
relative path without `..` traversal; missing directories and symlinks that
escape the checkout block scanning. `--ref` resolves a branch, tag, or commit
to an exact detached commit and sets `update_policy = "pin"`; omit it to track
the repository's default branch. The selected subpath is recorded in
`config.toml` and `lock.toml`, so changing it reports lock drift.
...
dalo source add platform git@github.com:example/monorepo.git --ref v1.2.0 --subpath tools/skills
...
Select catalog skills by stable frontmatter ID, slot name, catalog-relative path,
or a source-qualified `<source-id>:<slot-or-stable-id>` reference. The source ID
must match the catalog being selected.
...
| `sources[].subpath` | Optional directory inside a team checkout used as the inventory root. Must remain inside the checkout. |
...
| `sources[].selection` | Persisted catalog selections by stable ID, slot name, or catalog-relative path. Source-qualified command input is normalized before storage. Empty for local/team sources. |
...
| Field | Meaning |
| --- | --- |
| `sources[]` | Source ID, kind, path, optional subpath, and optional commit. |
...
| Record | Fields |
| --- | --- |
| `LockedSource` | `id`, `kind`, `path`, optional `subpath`, optional `commit` |
...
Generator execution is fail-closed behind an operating-system sandbox inherited
by every descendant. The sandbox permits filesystem writes only below the one
delivery staging directory. macOS uses the system Seatbelt launcher and denies
network operations. Linux requires a fully enforced Landlock ABI v4 domain and
denies TCP connection and bind operations. If the required sandbox cannot be
fully enforced, Dalo does not run the generator.

This is not a general read-confinement boundary: a generator can read files its
OS user can read, including the native runtime files that executable tools
need. Linux Landlock network controls do not cover UDP or UNIX-domain sockets.
Treat a generated-delivery approval as permission to run the exact staged tool
closure within these stated write and network limits, not as permission for
arbitrary code or as proof that its inputs are confidential.
