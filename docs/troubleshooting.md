...
| `blocked_winner_alternate_available` | A higher-precedence winner was blocked by its required closure while an approved lower-precedence alternate for the same slot remains shadowed. This advisory does not auto-promote the alternate or itself make `--check` fail; the related `required_blocked` diagnostic does. | Resolve the winner's blocked requirement(s), or give the approved alternate higher precedence with a lower `dalo source priority` value. |
...
team or catalog sources, do not edit Dalo's managed checkout: fix the upstream
repository, push the change, then run `dalo sync`.
...
| `source_provenance_mismatch` | error | Restore the intended catalog checkout. For a direct catalog, review `dalo --dry-run source refresh <id> --advance` before intentionally advancing its pin; for a manifest catalog, compare `dalo source list` with the declaring team's `dalo.toml`, then run `dalo sync`. |
