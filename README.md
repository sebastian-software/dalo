...
If a target folder already contains a real unmanaged skill with the same name, Dalo does not overwrite it. The managed skill is blocked with `blocked_by_same_name_skill`, and `status` explains what happened. `dalo resolve keep <id>` marks the slot as intentionally unmanaged so sync and status checks stay green; `dalo resolve unkeep <target>:<slot>` restores normal conflict handling.
